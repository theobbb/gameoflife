...
const CELL_COLOR = 'rgba(255, 255, 255, 0.9)';
...
export class Grid {
	...
	private steps: Record<number, () => (() => void) | void> = {
		...
		4: () => {
			const mid = Math.floor(BASE_GRID_SIZE / 2);
			this.grid[mid][mid] = 1;
			this.updateInterval = 2000; // slow motion
		...
		10: () => {
			...
			for (let y = startY; y < startY + 17; y++) {
				for (let x = startX; x < startX + 17; x++) {
					if (this.grid[y][x] === 0) {
						dead_cells.push([x, y]);
					}
				}
