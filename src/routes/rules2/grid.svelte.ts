...
export const GRID_COLOR = 'rgba(255, 255, 255, 0.3)';

const HOVER_SCALE = 0.9;
const HOVER_ACTIVE_COLOR = 'rgba(255, 255, 0, 0.3)';
...
export class Grid {
	...
	private steps: Record<number, () => (() => void) | void> = {
		...
		11: () => {
			...
			// Pattern layout (centered on grid):
			//   b o b   → (mid-1,mid-1)=dead, (mid,mid-1)=alive, (mid+1,mid-1)=dead
			//   o o o   → all alive
			//   o o b   → (mid+1,mid+1)=dead
			//
			// Target: top-right dead cell (mid+1, mid-1) — 3 live neighbours, will be born
			const mid = Math.floor(BASE_GRID_SIZE / 2);
			const targetX = mid + 1;
			const targetY = mid - 1;
