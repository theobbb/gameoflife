...
export class Engine {
	...
	private get_theme() {
		...
		const life = styles.getPropertyValue('--color-life').trim();
		...
		if (grid) this.currentColors.grid = this.parseCssColor(grid, 0.2);
		if (life) this.currentColors.blob = this.parseCssColor(life, 0.85);
	}
	...
	setTheme(themeName: string, duration = 600) {
		...
		const bgStr = styles.getPropertyValue('--color-bg').trim();
		const gridStr = styles.getPropertyValue('--color-grid').trim();
		const lifeStr = styles.getPropertyValue('--color-life').trim();
		...
		this.transition.targetColors.background = this.parseCssColor(bgStr, 1);
		this.transition.targetColors.grid = this.parseCssColor(gridStr, 0.2);
		this.transition.targetColors.blob = this.parseCssColor(lifeStr, 0.85);
