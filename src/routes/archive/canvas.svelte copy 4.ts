...
export function create_canvas(ref: HTMLCanvasElement) {
	...
	function syncVisualCells() {
		// Track active keys to identify dead cells later
		// Using a set of numbers is faster than strings
		...
		for (let row = 0; row < ROWS; row++) {
			for (let col = 0; col < COLS; col++) {
				...
				// Skip completely dead areas to save cycles
				...
				if (!isAlive && willBeAlive && isAnimating) {
					if (!visualCells.has(key)) {
						...
						visualCells.set(key, {
							gridX: col,
							gridY: row,
							x: targetX - vector.x * (BASE_CELL_SIZE / 2),
							y: targetY - vector.y * (BASE_CELL_SIZE / 2),
							vx: 0,
							vy: 0,
							targetX,
							targetY,
	...
	function updatePhysics(dt: number) {
		...
		// Optimization: Single loop for active cells
		for (const cell of visualCells.values()) {
			...
			cell.targetX =
				cell.gridX * BASE_CELL_SIZE + BASE_CELL_SIZE / 2 + Math.sin(cell.phase) * wobbleAmount;
			cell.targetY =
				cell.gridY * BASE_CELL_SIZE +
				BASE_CELL_SIZE / 2 +
				Math.cos(cell.phase * 1.3) * wobbleAmount;
			...
			if (!cell.dying) {
				...
				const pushDist = progress * (BASE_CELL_SIZE / 2);
				cell.targetX += vector.x * pushDist;
				cell.targetY += vector.y * pushDist;
			}
			...
			cell.vx = (cell.vx + dx * STIFFNESS * tSec) * Math.pow(DAMPING, tSec);
			cell.vy = (cell.vy + dy * STIFFNESS * tSec) * Math.pow(DAMPING, tSec);
