...
export class GameEngine {
	// --- STATE (Runes) ---
	// Exposed state that the UI might need to react to
	isPlaying = $state(false);
	zoom_level = $state(4);
	time_scale = $state(1.0);
	stats: {
		n_gens: number;
	} = $state({ n_gens: 0 });
	...
	mount(canvas: HTMLCanvasElement) {
		...
		// $effect(() => {
		// 	const pattern = page.data.pattern;
		// 	if (pattern) {
		// 		// Wrap in untracked if you don't want deep dependency tracking on the pattern object internals
	...
	private updatePhysics(dt: number, scale: number) {
		...
		for (const cell of this.visualCells.values()) {
			...
			if (!cell.dying) {
				...
				if (this.isAnimating && cell.scale < 1) cell.scale = this.lerp(0.5, 1, progress);
	...
	private drawMetaballs(startRow: number, endRow: number, startCol: number, endCol: number) {
		...
		// Optimization: Use standard loop instead of iterator
		for (const cell of this.visualCells.values()) {
			if (
	...
	private animate = (currentTime: number) => {
		...
		const effective_duration = ANIMATION_DURATION / this.time_scale;
		const effective_interval = this.UPDATE_INTERVAL / this.time_scale;

		if (this.isAnimating) {
			const elapsed = currentTime - this.animationStartTime;
			this.animationProgress = Math.min(elapsed / effective_duration, 1);
			if (this.animationProgress >= 1) this.completeTransition();
		}

		this.updateCamera();

		// 2. Pass scale to physics
		...
		if (
			this.isPlaying &&
			!this.isAnimating &&
			// Use effective_interval
			currentTime - this.lastUpdateTime > effective_interval
		) {
			this.updateGrid();
	...
	togglePlay = () => {
		...
		if (this.isPlaying) this.lastUpdateTime = performance.now();
		this.draw();
	};
	...
	drawPattern(rle_pattern: Pattern) {
		console.log('drawing...', rle_pattern);

		// --- FIX START ---
		// 1. Kill any active transition immediately
		this.isAnimating = false;
		this.animationProgress = 0;

		// 2. Reset the update timer so the new pattern doesn't
		// immediately tick if the game is currently playing.
		// It gives the user 100ms (or whatever UPDATE_INTERVAL is) to see the start state.
		this.lastUpdateTime = performance.now();
		// --- FIX END ---
	...
	private setupListeners() {
		...
		window.addEventListener('mouseup', this.onMouseUp); // Window ensures drag release works outside canvas
	...
	private onMouseDown = (event: MouseEvent) => {
		...
		this.safeSet(row, col, !this.grid[row][col]); // Initial click toggles
		this.syncVisualCells();
		this.draw();
	...
	private onMouseMove = (event: MouseEvent) => {
		if (!this.isMouseDown || !this.canvas_rect) return;
		const x = event.clientX - this.canvas_rect.left;
		const y = event.clientY - this.canvas_rect.top;
		const { col, row } = this.screenToGrid(x, y);

		// Dragging always makes cells alive (like painting)
		if (row >= 0 && row < this.ROWS && col >= 0 && col < this.COLS) {
...
export function init_engine() {
	return setContext(GAME_ENGINE_SYMBOL, new GameEngine());
}

export function get_engine() {
	const store = getContext<GameEngine>(GAME_ENGINE_SYMBOL);
