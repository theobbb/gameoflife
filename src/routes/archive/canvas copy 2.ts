			...
			for (let col = startCol; col < endCol; col++) {
				...
				if (isAnimating) {
					...
					if (isAlive && willBeAlive) {
						...
						const translateX = vector.x * progress;
						const translateY = vector.y * progress;

						// Shrink the cell as it dies
						const shrink = progress * 0.3; // shrink by 30%
						...
						const translateX = -vector.x * (1 - progress);
						const translateY = -vector.y * MAX_TRANSLATION * (1 - progress);
