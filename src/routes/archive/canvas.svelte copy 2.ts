...
export function create_canvas(ref: HTMLCanvasElement) {
	...
	function parseRLE(rleString: string): { pattern: boolean[][]; width: number; height: number } {
		...
		// Parse RLE encoded pattern
		const pattern: boolean[][] = Array(height)
			.fill(null)
	...
	/**
	 * Clear the grid and draw an RLE pattern
	 * @param rleString - RLE format pattern string
	 * @param centerX - Optional X position (defaults to center of grid)
	 * @param centerY - Optional Y position (defaults to center of grid)
	 */
	function drawPattern(rleString: string, centerX?: number, centerY?: number) {
		...
		// Draw the pattern onto the grid
		for (let row = 0; row < height; row++) {
			for (let col = 0; col < width; col++) {
				const gridRow = startRow + row;
				const gridCol = startCol + col;

				// Check bounds and wrap if needed
	...
	function test() {
		// Example: Gosper Glider Gun (a classic pattern that generates gliders infinitely)
